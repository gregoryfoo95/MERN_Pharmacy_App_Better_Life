...
export default function AvailabilityCard() {
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACK_END_URL}/api/user/${user._id}`);
  ...
  const handleAvailClick = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_BACK_END_URL}/api/user/${user._id}`, newUser, {
        headers: {
