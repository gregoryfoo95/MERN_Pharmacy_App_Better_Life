...
export default function StockPage() {
    ...
    try {
      medicines.map(async medicine => {
        ...
        if (newQty !== undefined && newQty !== originalQty) {
          await axios.put(`${import.meta.env.VITE_APP_BACK_END_URL}/api/stock/${medicine._id}/updatestock`, {
            quantity: newQty,
          });
        }
      });
  ...
  useEffect(function () {
    async function getAllStocks() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_APP_BACK_END_URL}/api/stock`);
