...
//       try {
//         const response = await axios.get(`http://localhost:3000/api/map/${id}`);
...
//     marker.bindPopup(
//       `Store Name: ${location.storeName}<br>Pharmacist: ${location.Pharmacy}`
//     );
...
function Directions({ zoom = 17 }) {
  ...
  const [location, setLocation] = useState(null); 
  const [destinationMarker, setDestinationMarker] = useState(null);
  const [routingControl, setRoutingControl] = useState(null); // New state variable
